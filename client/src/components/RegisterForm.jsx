...
const RegisterForm = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      Navigate("/link"); // Always redirect to link page after registration
    } catch (error) {
    ...
    >
      ...
      <div className="w-1/2 h-full flex items-center justify-center bg-black/40 backdrop-blur-sm">
        ...
        >
          ...
          {errors.name && (
            <span className="text-red-500 text-sm mb-4">{errors.name}</span>
          )}
          ...
          <button
            type="submit"
            disabled={!isValid}
            className="w-full py-2 rounded border mt-4 border-white/30 text-white bg-purple-400/10 hover:bg-purple-300 hover:text-black transition"
          >
            Register
          </button>
          {serverError && (
            <span className="text-red-500 text-sm mb-4">{serverError}</span>
          )}
